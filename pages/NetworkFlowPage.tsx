...
export const NetworkFlowPage = () => {
  ...
  const filteredTransactions = data.transactions.filter((tx: any) => {
    ...
    return (
      tx?.from_wallet?.toLowerCase().includes(term) ||
      tx?.to_wallet?.toLowerCase().includes(term) ||
      tx?.tx_id?.toLowerCase().includes(term)
    );
  });
      ...
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Raw ledger containing {data.transactions.length} immutable
            transaction records.
          </p>
        </div>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 font-mono text-slate-700 font-medium">
                      <span
                        ...
                        onClick={() =>
                          navigate(`/wallet/${tx.from_wallet}`)
                        }
